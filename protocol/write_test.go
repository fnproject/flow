...
func TestShouldWriteSimpleBlob(t *testing.T) {
	...
	for _, proto := range protos {
		t.Run(proto.name(), func(t *testing.T) {
			datum := model.NewBlobDatum(model.NewBlob("blobId", uint64(101), "text/plain"))
...
func TestShouldAddResultStatusToResult(t *testing.T) {
	for _, proto := range protos {
		t.Run(proto.name(), func(t *testing.T) {
			headers, _ := proto.writeResult(model.NewSuccessfulResult(model.NewBlobDatum(sampleBlobBody())))
			assert.Equal(t, ResultStatusSuccess, headers.Get(HeaderResultStatus))

			headers, _ = proto.writeResult(model.NewFailedResult(model.NewBlobDatum(sampleBlobBody())))
...
func TestShouldWriteStateDatum(t *testing.T) {
	for _, proto := range protos {
		t.Run(proto.name(), func(t *testing.T) {
			...
			headers, body := proto.writeDatum(stateDatum)
...
func sampleBlobBody() *model.BlobDatum {
	return model.NewBlob("blobId", 101, "text/plain")
}

type testProto interface {
	name() string
	writeResult(result *model.CompletionResult) (textproto.MIMEHeader, string)
	writeDatum(datum *model.Datum) (textproto.MIMEHeader, string)
}

type mimeProto struct{}
type httpProto struct{}

var protos = []testProto{
	&mimeProto{},
	&httpProto{},
}

func (m *mimeProto) name() string {
	return "Mime"
}

func (m *mimeProto) writeResult(result *model.CompletionResult) (textproto.MIMEHeader, string) {
	buf := new(bytes.Buffer)
	pw := multipart.NewWriter(buf)
	err := WriteResult(NewMultipartTarget(pw), result)
	if err != nil {
		panic(err)
	}
	pw.Close()
...
func (m *mimeProto) writeDatum(datum *model.Datum) (textproto.MIMEHeader, string) {
	buf := new(bytes.Buffer)
	pw := multipart.NewWriter(buf)
	err := WriteDatum(NewMultipartTarget(pw), datum)
	if err != nil {
		panic(err)
	}
	pw.Close()
...
func (m *httpProto) writeResult(result *model.CompletionResult) (textproto.MIMEHeader, string) {
	...
	return toMimeHeaders(mock.header), string(mock.buf.Bytes())
...
func toMimeHeaders(httpHeader http.Header) textproto.MIMEHeader {
	newHeader := textproto.MIMEHeader{}
